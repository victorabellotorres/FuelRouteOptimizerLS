from pathlib import Path
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
import numpy as np

# === 1. Cargar el CSV ===

outdir = Path("plots/experimento_2")
outdir.mkdir(parents=True, exist_ok=True)

df = pd.read_csv("../data/experimento2.csv")

# Columnas numéricas
cols_numericas = [
    "time_ms", "nodesExpanded", "heurIni", "heurFi", "costIni", "costFi",
    "profitIni", "profitFi", "AssignedPetitionsIni", "AssignedPetitionsFi",
    "trucksUsedIni", "trucksUsedFi", "totalKmIni", "totalKmFi"
]
df[cols_numericas] = df[cols_numericas].apply(pd.to_numeric, errors="coerce")

print(df["algorithmEI"].value_counts())

# === 2. Métricas derivadas ===
df["qualityFi"] = df["profitFi"] - df["costFi"]
df["qualityIni"] = df["profitIni"] - df["costIni"]

df["deltaQualityFi"] = df["qualityFi"] - df["qualityIni"]
df["deltaHeurFi"] = df["heurFi"] - df["heurIni"]
df["deltaCostFi"] = df["costFi"] - df["costIni"]
df["deltaProfitFi"] = df["profitFi"] - df["profitIni"]
df["deltaAssignedPetitionsFi"] = df["AssignedPetitionsFi"] - df["AssignedPetitionsIni"]

# === 3. Visualizaciones ===
sns.set_theme(style="whitegrid")

# --- Bar plots ---
metrics = ["qualityFi", "qualityIni", "heurFi", "costFi", "profitFi", "nodesExpanded", "time_ms", "AssignedPetitionsFi"]
for metric in metrics:
    plt.figure(figsize=(9,5))
    sns.barplot(data=df, x="algorithmEI", y=metric, estimator="mean", ci=95)
    plt.title(f"{metric} medio por estrategia de inicialización")
    plt.xlabel("algorithmEI")
    plt.ylabel(metric)
    plt.tight_layout()
    plt.savefig(outdir / f"barplot_{metric}.png", dpi=300)
    plt.close()

# --- Winrate plots (con sentido real) ---
good_when_higher = ["qualityFi", "qualityIni", "heurFi", "profitFi", "AssignedPetitionsFi"]
good_when_lower  = ["costFi", "nodesExpanded", "time_ms"]

for metric in metrics:
    gmean = df.groupby(["iteration", "algorithmEI"])[metric].mean().reset_index()

    if metric in good_when_higher:
        winners_per_op = gmean.loc[gmean.groupby("iteration")[metric].idxmax()].set_index("iteration")["algorithmEI"]
    elif metric in good_when_lower:
        winners_per_op = gmean.loc[gmean.groupby("iteration")[metric].idxmin()].set_index("iteration")["algorithmEI"]
    else:
        raise ValueError(f"Métrica {metric} no clasificada correctamente")

    win_rate_macro = (winners_per_op.value_counts(normalize=True) * 100).round(1)
    print(f"\nWin-rate (macro por operación) para {metric}:")
    print(win_rate_macro)

    plt.figure(figsize=(7,4))
    win_rate_macro.plot(kind="bar")
    plt.title(f"Ganador por operación (métrica: {metric})")
    plt.ylabel("% operaciones ganadas")
    plt.xlabel("algorithmEI")
    plt.tight_layout()
    plt.savefig(outdir / f"winrate_macro_{metric}.png", dpi=300)
    plt.close()

# --- Boxplots de deltas ---
delta_metrics = ["deltaQualityFi", "deltaHeurFi", "deltaCostFi", "deltaProfitFi", "deltaAssignedPetitionsFi"]
for metric in delta_metrics:
    plt.figure(figsize=(9,5))
    sns.boxplot(data=df, x="algorithmEI", y=metric, showfliers=True)
    sns.stripplot(data=df, x="algorithmEI", y=metric, size=3, alpha=0.35, color="k")
    plt.title(f"Distribución de {metric} por estrategia de inicialización")
    plt.xlabel("algorithmEI")
    plt.ylabel(metric)
    plt.tight_layout()
    plt.savefig(outdir / f"boxplot_{metric}.png", dpi=300)
    plt.close()

print("\nAnálisis completado. Resultados guardados en:", outdir.resolve())
