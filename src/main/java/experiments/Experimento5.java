...
public class Experimento5 {
    ...
    public static final int NUM_CENTROS_DISTRIBUCION[] = {10, 5};
    public static final int NUM_GASOLINERAS = 100;
    public static final int NUM_CAMIONES_POR_CENTRO[] = {1, 2};
    ...
    private static final String OUTPUT_FILE = "data/experimento5.csv";
    ...
    public void run(int[] SEEDS) throws Exception {
        ...
        for (int j = 0; j < NUM_CENTROS_DISTRIBUCION.length; j++) {
            ...
            System.out.println("Numero de camiones por centro: " + NUM_CAMIONES_POR_CENTRO[j]);
            ...
            for (int i = 0; i < SEEDS.length; i++) {
                ...
                CentrosDistribucion centrosDistribucion = new CentrosDistribucion(NUM_CENTROS_DISTRIBUCION[j], NUM_CAMIONES_POR_CENTRO[j], SEED);
                ...
                for (int k = 0; k < iteraciones_aleatorio; k++) {
                    ...
                    exportarDatos(estadoInicial, hInicial, estadoFinal, ALGORITMO_ESTADO_INICIAL, "HillClimbing", HEURISTICA, hFinal, SEED, i, nodosExpandidos, end - start, OPERATORS, NUM_CAMIONES_POR_CENTRO[j]);
    ...
    private void exportarDatos(P1Board estadoInicial, double hInicial, P1Board estadoFinal, String algoritmoEstadoIni, String algorithmSearch, String funcionHeuristica ,double hFinal, int seed, int iteracion, String nodosExpandidos, long timeMili, boolean[] operators, int camionesPorCentro) throws IOException {
        BoardMetrics metricsInicial = estadoInicial.getMetrics();
        BoardMetrics metricsFinal = estadoFinal.getMetrics();

        File file = new File(OUTPUT_FILE);
        boolean writeHeader = !file.exists() || file.length() == 0;
        String header = "iteration,seed,time_ms,algorithmEI,algorithmSearch,op1,op2,op3,op4,nodesExpanded,validInicial,validFinal,heurFunction,heurIni,heurFi,costIni,costFi,profitIni,profitFi,AssignedPetitionsIni,AssignedPetitionsFi,trucksUsedIni,trucksUsedFi,totalKmIni,totalKmFi,centrosDist,camionesPorCentros\n";
        ...
        try (Writer out = new OutputStreamWriter(new FileOutputStream(file, true), StandardCharsets.UTF_8)) {
            ...
            try (CSVPrinter printer = new CSVPrinter(out, CSVFormat.DEFAULT)) {
                printer.printRecord(
                        ...
                        estadoInicial.getCamiones().length / camionesPorCentro,
                        camionesPorCentro
                );
                printer.flush();
            }
